...
void ScrCmd_createmon(struct ScriptContext *ctx)
{
    ...
    else if (FlagGet(FLAG_SYS_POKEMON_GET) && RandomizerFeatureEnabled(RANDOMIZE_FIXED_MON))
    {
        species = RandomizeMon(RANDOMIZER_REASON_FIXED_ENCOUNTER, GetRandomizerOption(RANDOMIZER_OPTION_SPECIES_MODE), Random32(), species);
    }
#endif

    // Match ScriptGiveMon: the monotype challenge owns the first starter choice,
    // after which explicitly tagged gifts continue to use the gift randomizer.
    if (side == 0 && isStarterOrGift && !FlagGet(FLAG_SYS_POKEMON_GET) && IsOneTypeChallengeActive())
        species = GetStarterPokemon(VarGet(VAR_STARTER_MON));
